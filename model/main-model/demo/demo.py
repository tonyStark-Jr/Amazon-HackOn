...
def main(
    ...
    #         aug_input.image.transpose(2, 0, 1))).to(device)
    ...
    #     print("hello")
    #     print(output['label_names'])
    #     if output_pth:
    #         torch.save(output, osp.join(output_dir, base_filename + '.pth'))

    #     # visualize output
    #     instances = output['instances']
    #     boxes, pred_classes, scores = filter_boxes(
    #         instances, threshold=threshold)
    ...
    #             box_id_indexes = (
    #                 pred_classes == c).nonzero().flatten().tolist()
    ...
    #     output = to_pil_image(draw_bounding_boxes(torch.as_tensor(image).permute(
    #         2, 0, 1), boxes, labels=[label_names[cid] for cid in pred_classes.tolist()], colors=colors))
